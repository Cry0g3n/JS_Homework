    ...
    results.addEventListener('click', function (e) {
        if (e.target.getAttribute('data-role') === 'playback') {
            ...
            if (currentItem === playingItem) {
                ...
                if (!globalPlayer.paused) {
                    onPause();
                    let progressBar = playingItem.querySelector('.progress-bar');
        ...
        else if (e.target.className.includes('progress')) {
            let currentItem = e.target.parentNode.parentNode.parentNode;

            if (currentItem === playingItem) {
                let progressContainer = playingItem.querySelector(".progress"),
                    offsetX = e.offsetX,
                    progressContainerWidth = progressContainer.offsetWidth,
                    percent = parseInt(offsetX / progressContainerWidth * 100);

                globalPlayer.currentTime = parseInt(globalPlayer.duration * percent / 100);
            }
        }
    }, true);
